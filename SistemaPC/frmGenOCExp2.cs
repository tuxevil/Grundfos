namespace SistemaPC
{
    ...
    public class frmGenOCExp2 : Form
    {
        ...
        public frmGenOCExp2()
        {
            base.Closed += new EventHandler(this.frmGenOCExp2_Closed);
            base.Load += new EventHandler(this.frmGenOCExp2_Load);
            base.Click += new EventHandler(this.frmGenOCExp2_Click);
            ...
            try
            {
                ...
                connection.Open();
                SqlCommand command = new SqlCommand("SELECT * from " + Variables.gTermi + "TmpOCompraExp as PC1TmpOCompraExp where Seleccion=1", connection);
                command.CommandTimeout = 500;
                this.Adap1 = new SqlDataAdapter();
                this.Adap1.SelectCommand = command;
                this.DS.Clear();
                this.Adap1.Fill(this.DS, "PC1TmpOCompraExp");
                this.Informe.Load(Application.StartupPath + @"\GenOCompraExp.rpt");
                this.Informe.SetDataSource(this.DS);
                connection.Close();
                FormulaFieldDefinition definition = this.Informe.DataDefinition.FormulaFields["MetEnv"];
                definition.Text = "'M\x00e9todo Env\x00edo: " + Strings.Trim(Variables.gDescMetEnt) + "'";
        ...
        private void frmGenOCExp2_Click(object sender, EventArgs e)
        {
        }

        private void frmGenOCExp2_Closed(object sender, EventArgs e)
        {
            new frmGenOCExp1().Show();
        }

        private void frmGenOCExp2_Load(object sender, EventArgs e)
        {
        }
        ...
        {
            ResourceManager manager = new ResourceManager(typeof(frmGenOCExp2));

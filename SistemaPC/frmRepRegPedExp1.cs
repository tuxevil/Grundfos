    ...
    public class frmRepRegPedExp1 : Form
    {
        ...
        public frmRepRegPedExp1()
        {
            ...
            try
            {
                ...
                FormulaFieldDefinition definition = formulaFields["desdefechaent"];
                if (StringType.StrCmp(Variables.gDesdeFechaEnt, Strings.Space(0), false) != 0)
                {
                    definition.Text = "'" + Strings.Format(DateType.FromString(Variables.gDesdeFechaEnt), "dd/MM/yyyy") + "'";
                }
                else
                {
                    definition.Text = "''";
                }
                FormulaFieldDefinition definition2 = formulaFields["hastafechaent"];
                if (StringType.StrCmp(Variables.gHastaFechaEnt, Strings.Space(0), false) != 0)
                {
                    definition2.Text = "'" + Strings.Format(DateType.FromString(Variables.gHastaFechaEnt), "dd/MM/yyyy") + "'";
                }
                else
                {
                    definition2.Text = "''";

        ...
        private void cmbAceptar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (num = 0; num <= num10; num++)
                {
                    ...
                    if (reader.Read())
                    {
                        if (ObjectType.ObjTst(row["CantRec"], ObjectType.SubObj(reader["PC03010"], reader["PC03011"]), false) == 0)
                        {
                            ...
                            double num6;
                            double num7;
                            string str3 = (((((StringType.FromObject(ObjectType.StrCatObj(ObjectType.StrCatObj(StringType.FromObject(ObjectType.StrCatObj(ObjectType.StrCatObj(StringType.FromObject(row["NroOC"]), reader["PC03002"]), Strings.Space(6 - Strings.Len(RuntimeHelpers.GetObjectValue(reader["PC03002"]))))), row["Codigo"]), Strings.Space(0x23 - Strings.Len(RuntimeHelpers.GetObjectValue(row["Codigo"]))))) + Strings.Format(RuntimeHelpers.GetObjectValue(row["CantRec"]), "00000000000.00000000") + Strings.Format(DateAndTime.Now, "ddMMyyyy")) + Variables.gDespacho + Strings.Space(0x19 - Strings.Len(Variables.gDespacho))) + Strings.Format(Variables.gFechaImp, "ddMMyyyy")) + Variables.gPaisOrigen + Strings.Space(11 - Strings.Len(Variables.gPaisOrigen))) + Variables.gCodAduana + Strings.Space(10 - Strings.Len(Variables.gCodAduana))) + Strings.Format(DateAndTime.Now, "ddMMyyyy") + "\r" + "\n";
                            string vRight = StringType.FromObject(reader["PC03002"]);
                            reader.Close();
                    ...
                    {
                        str7 = StringType.FromObject(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj((((((("insert into Recepcion select NroBulto,NroOC,NroLinea,Codigo,Desc1,Desc2,0,CantRec,'" + Strings.Format(DateAndTime.Now, "MM/dd/yyyy") + "',0,0,0,'") + Variables.gDespacho + "','") + Strings.Format(Variables.gFechaImp, "MM/dd/yyyy") + "','") + Variables.gCodAduana + "','") + Variables.gPaisOrigen + "','") + Variables.gPackList + "' from Bultos where PackList='") + Variables.gPackList + "' and NroOC='", row["NroOC"]), "' and Codigo='"), row["Codigo"]), "'"));
                        reader.Close();
                        command = new SqlCommand(str7, connection2);
                        command.Transaction = transaction;
                        num8 = command.ExecuteNonQuery();
                ...
                for (num = 0; num <= num9; num++)
                {
                    ...
                    if (reader.Read())
                    {
                        string inputStr = StringType.FromObject(reader["ordnb"]);
                        string str = StringType.FromObject(reader["lignb"]);
                        reader.Close();
                        command = new SqlCommand(StringType.FromObject(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj("SELECT * from Recepcion where NroBulto='", row["NroBulto"]), "' and Codigo='"), row["Codigo"]), "' and NroOC='"), Strings.Format(Conversion.Val(inputStr), "0000000000")), "'")), connection2);
                        reader = command.ExecuteReader();
                        if (!reader.Read())
                        {
                            reader.Close();
                            command = new SqlCommand("SELECT PC03001,PC03002,PC03005,PC03006,PC03007,PC03010,PC03011,PC03016 FROM dbo.PC030100 where PC03001='" + inputStr + "' and PC03002='" + str + "' and PC03010<>0 and PC03010-PC03011<>0", connection);
                            reader = command.ExecuteReader();
                            if (reader.Read())
                            {
                                str7 = StringType.FromObject(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(("insert into Recepcion select NroBulto,'" + Strings.Format(Conversion.Val(inputStr), "0000000000") + "','") + Strings.Format(Conversion.Val(str), "000000") + "',Codigo,Desc1,Desc2,", ObjectType.SubObj(reader["PC03010"], reader["PC03011"])), ",0,'"), Strings.Format(DateAndTime.Now, "MM/dd/yyyy")), "',0,0,0,Despacho,FechaImp,CodAduana,PaisOrigen,PackList from Recepcion where NroOC='"), row["NroOC"]), "' and Codigo='"), row["Codigo"]), "' and NroBulto='"), row["NroBulto"]), "'"));
                                reader.Close();
                                command = new SqlCommand(str7, connection2);
                                num8 = command.ExecuteNonQuery();
                            }
                            ...
                            {
                                str7 = StringType.FromObject(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj(ObjectType.StrCatObj((("insert into Recepcion select NroBulto,'" + Strings.Format(Conversion.Val(inputStr), "0000000000") + "','" + Strings.Format(Conversion.Val(str), "000000")) + "',Codigo,Desc1,Desc2,0,0,'") + Strings.Format(DateAndTime.Now, "MM/dd/yyyy") + "',0,0,0,Despacho,FechaImp,CodAduana,PaisOrigen,PackList from Recepcion where NroOC='", row["NroOC"]), "' and Codigo='"), row["Codigo"]), "' and NroBulto='"), row["NroBulto"]), "'"));
                                reader.Close();

namespace SistemaPC
{
    ...
    public class frmRecOCManual : Form
    {
        ...
        private void cmbAceptar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (reader.Read())
                {
                    reader.Close();
                    connection.Close();
                    if (Interaction.MsgBox("Este despacho ya fue procesado, continua?", MsgBoxStyle.YesNo, "Operador") == MsgBoxResult.No)
                    {
        ...
        {
            if (Interaction.MsgBox("Los datos del despacho son correctos?", MsgBoxStyle.YesNo, "Operador") == MsgBoxResult.No)
            {
                this.GroupBox2.Enabled = true;
                this.txtDespacho.Focus();
            }
        ...
        private void cmbIngresar_Click(object sender, EventArgs e)
        {
            ...
            {
                Interaction.MsgBox("Debe ingresar nro.factura proveedor", MsgBoxStyle.Critical, "Operador");
                this.editNroFC.Focus();
            }
            else
            {
        ...
        {
            bool flag = false;
            if ((this.DS.Tables["FCProv"].Rows.Count != 0) && (Interaction.MsgBox("Desea eliminar la factura", MsgBoxStyle.YesNo, "Operador") == MsgBoxResult.Yes))
            {
                SqlConnection connection;
                string str3;
                try
        ...
        {
            bool flag = false;
            this.dtpFechaRec.Value = DateAndTime.Now;
            try
            {
            ...
            this.dtpFechaImp.Value = DateAndTime.Now;
        ...
        private void txtCodAduana_LostFocus(object sender, EventArgs e)
        {
            if (StringType.StrCmp(this.txtCodAduana.Text, Strings.Space(0), false) != 0)
            {
                ...
                {
                    Interaction.MsgBox("C\x00f3digo Aduana inexistente", MsgBoxStyle.Information, "Operador");
                    this.txtDescAdu.Text = "";
                    reader.Close();
                }
            }

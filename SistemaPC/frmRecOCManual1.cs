    ...
    public class frmRecOCManual1 : Form
    {
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            if (StringType.StrCmp(this.mTipo, "A", false) == 0)
            {
                ...
                {
                    Interaction.MsgBox("Debe ingresar Nro.OC", MsgBoxStyle.Critical, "Operador");
                    this.editNroOC.Focus();
                    return;
                }
                if (!Information.IsNumeric(this.editNroOC.Text))
                {
                    Interaction.MsgBox("Nro.OC inv\x00e1lido", MsgBoxStyle.Critical, "Operador");
                    this.editNroOC.Focus();
                    return;
                }
                if (StringType.StrCmp(this.editNroLinea.Text, Strings.Space(0), false) == 0)
                {
                    Interaction.MsgBox("Debe ingresar L\x00ednea", MsgBoxStyle.Critical, "Operador");
                    this.editNroLinea.Focus();
                    return;
                }
                if (!Information.IsNumeric(this.editNroLinea.Text))
                {
                    Interaction.MsgBox("L\x00ednea inv\x00e1lido", MsgBoxStyle.Critical, "Operador");
                    this.editNroLinea.Focus();
                    return;
                }
                if (StringType.StrCmp(this.txtCodigo.Text, Strings.Space(0), false) == 0)
                {
                    Interaction.MsgBox("Debe ingresar Producto", MsgBoxStyle.Critical, "Operador");
                    this.txtCodigo.Focus();
                    return;
                }
                if (StringType.StrCmp(this.txtDesc1.Text, Strings.Space(0), false) == 0)
                {
                    Interaction.MsgBox("Producto inexistente", MsgBoxStyle.Critical, "Operador");
                    this.txtCodigo.Focus();
                    return;
                }
                if (StringType.StrCmp(this.editCantRec.Text, Strings.Space(0), false) == 0)
                {
                    Interaction.MsgBox("Debe ingresar Cantidad Recibida", MsgBoxStyle.Critical, "Operador");
                    this.editCantRec.Focus();
                    return;
                }
                if (!Information.IsNumeric(this.editCantRec.Text))
                {
                    Interaction.MsgBox("Cantidad Recibida inv\x00e1lida", MsgBoxStyle.Critical, "Operador");
                    this.editCantRec.Focus();
                    return;
                }
            }
        ...
        private void editCantRec_LostFocus(object sender, EventArgs e)
        {
            ...
            {
                this.mFecEntOPend = DateAndTime.Now;
        ...
        private void txtCodigo_LostFocus(object sender, EventArgs e)
        {
            ...
            if (StringType.StrCmp(this.txtCodigo.Text, Strings.Space(0), false) != 0)
            {
                ...
                try
                {
                    ...
                    {
                        Interaction.MsgBox("Producto inexistente", MsgBoxStyle.Information, "Operador");
                        this.txtDesc1.Text = "";
                        this.txtDesc2.Text = "";
                        reader.Close();
                        return;

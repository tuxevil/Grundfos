    ...
    <br />
     <div ID="divMarked" runat="server" class="actions">
            ...
            <ul runat="server" id="ulCheck">
                <li id="MarkedAll"><a id="markAll" class="link" href="#Actions" >Marcar todos</a></li>
                <li> | </li>
...
<center>
    ...
    <asp:AsyncPostBackTrigger ControlID="ddlPageSize" />
        ...
        <input id="hidIsAdmin" runat="server" type="hidden" value="false" />
...
{
    var id = $(this).parents("tr").eq(0).attr("id").replace(/pp/,"");
    var status = $(this).parents("tr").eq(0).attr("quotestatus").replace(/pp/,"");
    if((status != '3' || status != '4') && $("#<%=hidIsAdmin.ClientID%>").text()!= 'false')

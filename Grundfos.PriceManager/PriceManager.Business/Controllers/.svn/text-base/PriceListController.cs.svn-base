...
namespace PriceManager.Business
{
    public class PriceListController : AbstractNHibernateDao<PriceList, int>
    {
        ...
        public PriceList Create(string name, string description, Lookup type, Lookup saleCondition, PriceListStatus status, Frequency? frequency, PriceGroup pG, Currency currency)
        {
        ...
        public PriceList Modify(PriceList pl, string name, string description, Lookup type, Lookup saleCondition, PriceListStatus status, Frequency? frequency, Currency currency)
        {
            if(pl.ID != 0)
                pl = GetById(pl.ID);
        ...
        public IList<PriceList> GetPriceLists(string description, Frequency? frequency, DateTime? date, Distributor distributor, Lookup incoterm, PublishListStatus? status, GridState gridState, PriceGroup priceGroup, out int totalRecords, Lookup type, CatalogPage page)
        {
        ...
        private ICriteria GetPriceListsCriteria(string description, Frequency? frequency, DateTime? date, Distributor distributor, Lookup incoterm, PublishListStatus? status, PriceGroup priceGroup, Lookup type, CatalogPage page)
        {
            ICriteria crit = GetCriteria();

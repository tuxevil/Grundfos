...
namespace Grundfos.ScalaConnector.Controllers
{
    public class SaleOrderItemController : AbstractNHibernateDao<SaleOrderItem, string>
    {
        ...
        {
            ICriteria crit = GetCriteria();
            crit.Add(Expression.GtProperty("Quantity", "QuantityDelivery"));
            crit.CreateCriteria("SaleOrder").Add(Expression.Ge("Date", startdate)).Add(Expression.Not(Expression.InsensitiveLike("Id", "00003", MatchMode.Start))).SetFetchMode("Customer", FetchMode.Join);
            crit.SetFetchMode("Product", FetchMode.Join);
            crit.AddOrder(new Order("SaleOrderId", true));

            return crit.List<SaleOrderItem>() as List<SaleOrderItem>;
        }

        public List<SaleOrderItem> CompletedSaleOrder(DateTime startdate)
        {
            ICriteria crit = GetCriteria();

            crit.Add(new EqPropertyExpression("Quantity", "QuantityDelivery"));
            crit.CreateCriteria("SaleOrder").Add(new GeExpression("Date", startdate));
            crit.SetFetchMode("Product", FetchMode.Join);
        ...
        public int SaleOrderCount(DateTime startdate)
        {
            ICriteria crit = GetCriteria();

            crit.CreateCriteria("SaleOrder").Add(new GeExpression("Date", startdate));

            return crit.List<SaleOrderItem>().Count;
        }

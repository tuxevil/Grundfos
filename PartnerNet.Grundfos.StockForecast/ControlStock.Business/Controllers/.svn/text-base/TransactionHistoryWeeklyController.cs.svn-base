...
namespace PartnerNet.Business
{
    public class TransactionHistoryWeeklyController : AbstractNHibernateDao<TransactionHistoryWeekly, int>
    {
        ...
        private IList<AlertReposition> GetAlert6(int week, int year, string where)
        {
            ...
            where += " and P.CountryCode not in ('" + Config.ArgentineCountryCode1 + "', '" + Config.ArgentineCountryCode2 + "') and P.Disarm = 0)";

            string query = "select P.Id as Id, P.RepositionLevel as RepoLevel, P.ProductCode as ProductCode, P.AlternativeProduct as AlternativeProduct, P.AlternativeDate as AlternativeDate,";
            ...
            query += " having sum(THW.Sale) > 0 and P.RepositionLevel >= 0";


            IQuery q = NHibernateSession.CreateSQLQuery(query).AddScalar("Id", NHibernateUtil.Int32).AddScalar("RepoLevel", NHibernateUtil.Int32).AddScalar("ProductCode", NHibernateUtil.String).AddScalar("AlternativeProduct", NHibernateUtil.String).AddScalar("AlternativeDate", NHibernateUtil.DateTime).AddScalar("Sales", NHibernateUtil.Int32).AddScalar("Months", NHibernateUtil.Int32);

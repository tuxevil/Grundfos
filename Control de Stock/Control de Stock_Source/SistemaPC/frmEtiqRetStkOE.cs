namespace SistemaPC
{
    ...
    public class frmEtiqRetStkOE : Form
    {
        ...
        private void btnContinuar_Click(object sender, EventArgs e)
        {
            ...
            FileSystem.FileOpen(num, @"C:\PrepPed.txt", 2, -1, -1, -1);
            FileSystem.PrintLine(num, new object[] { "\x0002c0000" });
            FileSystem.PrintLine(num, new object[] { "\x0002e" });
            FileSystem.PrintLine(num, new object[] { "\x0002M1407" });
            FileSystem.PrintLine(num, new object[] { "\x0002a" });
            FileSystem.PrintLine(num, new object[] { "\x0002RN" });
            FileSystem.PrintLine(num, new object[] { "\x0002s" });
            FileSystem.PrintLine(num, new object[] { "\x0002V0" });
            FileSystem.PrintLine(num, new object[] { "\x0002O0110" });
            FileSystem.PrintLine(num, new object[] { "\x0002Kf0038" });
            FileSystem.PrintLine(num, new object[] { "\x0002L" });
            FileSystem.PrintLine(num, new object[] { "D11" });
            FileSystem.PrintLine(num, new object[] { "A0" });
            FileSystem.PrintLine(num, new object[] { "Q0001" });
            FileSystem.PrintLine(num, new object[] { "491100500140094Cliente: " });
            FileSystem.PrintLine(num, new object[] { "491100500860096" + this.txtNomCli.Text });
            FileSystem.PrintLine(num, new object[] { "491100500140120Fecha Entrega      : " + this.txtFechaEntPed.Text });
            FileSystem.PrintLine(num, new object[] { "491100500140146O.Venta Nro.        : " + this.txtNroOV.Text });
            FileSystem.PrintLine(num, new object[] { "491100700140186Lista Recogida Nro.  : " + this.txtNroListaRec.Text });
            FileSystem.PrintLine(num, new object[] { "491100700140226O.Ensamblado Nro.  : " + Variables.gNroOE });
            FileSystem.PrintLine(num, new object[] { "4e5509000140329A" + this.txtNroListaRec.Text + Variables.gNroOE });
            FileSystem.PrintLine(num, new object[] { "E" });
            FileSystem.FileClose(new int[] { num });
            Interaction.Shell(@"C:\PrepPed.bat", 2, false, -1);
            this.GroupBox1.Visible = false;
            this.editNroOE.Enabled = true;
            this.editNroOE.Text = "";
            this.cmbSalir.Enabled = true;
        ...
        private void cmbAceptar_Click(object sender, EventArgs e)
        {
            ...
            {
                Interaction.MsgBox("Debe indicar o.ensamblado", 0x10, "Operador");
                this.editNroOE.Focus();
            }
            else
            {
                ...
                if (((int) -(reader.HasRows > false)) == 0)
                {
                    reader.Close();
                    connection.Close();
                    Interaction.MsgBox("O.ensamblado inexistente", 0x10, "Operador");
                    this.editNroOE.Enabled = true;
                    this.cmbAceptar.Enabled = true;
                    this.cmbSalir.Enabled = true;
                    this.editNroOE.Text = "";
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (StringType.StrCmp(Strings.Trim(StringType.FromObject(reader["OR03051"])), "", false) == 0)
                        {
                            reader.Close();
                            connection.Close();
                            Interaction.MsgBox("Esta O.ensamblado no tiene lista de recogida generada", 0x10, "Operador");
                            this.editNroOE.Enabled = true;
                            this.cmbAceptar.Enabled = true;
                            this.cmbSalir.Enabled = true;
                            this.editNroOE.Text = "";

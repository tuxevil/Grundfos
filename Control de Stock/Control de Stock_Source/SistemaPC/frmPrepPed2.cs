namespace SistemaPC
{
    ...
    public class frmPrepPed2 : Form
    {
        ...
        private void cmbAceptar_Click(object sender, EventArgs e)
        {
            ...
            {
                Interaction.MsgBox("No puede imprimir etiqueta de esta orden", 0x10, "Operador");
                this.cmbSalir.Focus();
            }
            else if (StringType.StrCmp(this.editCantEti.Text, Strings.Space(0), false) == 0)
            {
                Interaction.MsgBox("Debe ingresar cantidad de etiquetas a imprimir", 0x10, "Operador");
                this.editCantEti.Focus();
            }
            else if (!Information.IsNumeric(this.editCantEti.Text))
            {
                Interaction.MsgBox("Cantidad de etiquetas a imprimir debe ser num\x00e9rica", 0x10, "Operador");
                this.editCantEti.Focus();
            }
            else
            {
                ...
                if (Variables.gTipoOV != 6)
                {
                    ...
                    FileSystem.FileOpen(num2, @"C:\PrepPed.txt", 2, -1, -1, -1);
